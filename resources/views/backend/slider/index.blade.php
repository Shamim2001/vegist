                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <img class="img-thumbnail" alt="" width="200"
                                src="{{ getAssetUrl($slider->background) }}">
                        </td>
                        <td>{{ $slider->title }}</td>
                        <td class="text-capitalize">{{ $slider->alignment }}</td>
                        <td>
                            <div class="hstack gap-3 flex-wrap">
                                <a href="{{ route('slider.edit', $slider) }}" class="link-success fs-15"><i
                                        class="ri-edit-2-line"></i></a>

                                <a href="javascript:void(0)" class="link-danger fs-15 waves-effect waves-light ms-2"
                                    onclick="deleteRecord({{ $slider->id }})"><i class="ri-delete-bin-line"></i></a>


                                <form id="delete-form-{{ $slider->id }}"
                                    action="{{ route('slider.edit', $slider->id) }}" method="POST"
                                    style="display: none">
                                    @csrf
                                    @method('DELETE')
                                </form>
